...
template <template <auto...> typename scope> struct counter
{
  ...
  {
    int ii = 0;
    while(is_complete_type(substitute(
        ^^scope, {
...
template <template <auto...> typename scope, bool is_unique = true> struct type_registry
{
  ...
  {
    auto n = size(u);
    return std::views::iota(0, n) | std::views::transform([](int ii) {
             auto impl = substitute(
                 ^^scope, {
                              std::meta::reflect_constant(ii)});
             return type_of(
                 nonstatic_data_members_of(impl, std::meta::access_context::unchecked()).front());
           });
  }
