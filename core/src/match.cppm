export module reflex.core:match;
...
template <auto return_value>
inline constexpr auto ignore_r = [](auto&&...) { return return_value; };
} // namespace patterns

template <typename T, typename... Fns>
inline constexpr decltype(auto) operator|(T&& v, match<Fns...>&& m)
{
  return visit(fwd(m), fwd(v));
}

template <typename... Ts, typename... Fns>
inline constexpr decltype(auto) operator|(std::tuple<Ts...>&& v, match<Fns...>&& m)
{
  return std::apply([&m](auto&&... values) { return visit(fwd(m), fwd(values)...); }, fwd(v));
}
