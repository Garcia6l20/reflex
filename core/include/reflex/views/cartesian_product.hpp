...
REFLEX_EXPORT namespace reflex
{
  ...
  public:
    struct iterator
    {
      ...
      constexpr iterator(const cartesian_product_view* p, std::size_t i) : parent_(p), index_(i)
      {
  ...
  public:
    ...
    constexpr explicit cartesian_product_view(R base) : base_(std::move(base))
    {}
  ...
  template <std::size_t N> struct cartesian_product_fn
  {
    ...
    {
      return cartesian_product_view<N, std::views::all_t<R>>(std::views::all(std::forward<R>(r)));
    }

    template <std::ranges::viewable_range R>
    friend constexpr auto operator|(R&& r, const cartesian_product_fn& self)
    {
